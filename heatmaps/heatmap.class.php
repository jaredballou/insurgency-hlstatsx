...
class Heatmap {
	...
	public static function generate($code, $map, $mode) {
		...
		//              if (($mapinfo[$code][$map]['flipx']) && ($mapinfo[$code][$map]['xoffset'] > 0)) $mapinfo[$code][$map]['xoffset'] = $mapinfo[$code][$map]['xoffset'] * -1;
		//              if (($mapinfo[$code][$map]['flipy']) && ($mapinfo[$code][$map]['yoffset'] > 0)) $mapinfo[$code][$map]['yoffset'] = $mapinfo[$code][$map]['yoffset'] * -1;

		//		$x = round(abs($row['pos_x'] - $mapinfo[$code][$map]['xoffset']) / $maps[$map]['overview']['scale']);
		//		$y = round(abs($row['pos_y'] - $mapinfo[$code][$map]['yoffset']) / $maps[$map]['overview']['scale']);
//				$x = round(abs(($row['pos_victim_x'] - ($mapinfo[$code][$map]['xoffset']*$xm) / $mapinfo[$code][$map]['scale']))-1);
//				$y = round(abs(($row['pos_victim_y'] - ($mapinfo[$code][$map]['yoffset']*$ym) / $mapinfo[$code][$map]['scale']))-1);
		//var_dump($x);
		//var_dump($y);

			$xm = ($mapinfo[$code][$map]['flipx']) ? -1 : 1;
			$ym = ($mapinfo[$code][$map]['flipy']) ? -1 : 1;
		//              if ($mapinfo[$code][$map]['flipy']) $row['pos_y'] = $row['pos_y'] * -1;
		//var_dump($row);
			if (isset($row['killtype'])) {
				$pos_x = $row['pos_victim_x'];
				$pos_y = $row['pos_victim_y'];
			} else {
				$pos_x = $row['pos_x'];
				$pos_y = $row['pos_y'];
			}
...
//exit;
		...
		if ($mapinfo[$code][$map]['thumbw'] > 0 && $mapinfo[$code][$map]['thumbh'] > 0) {
			...
			imagepng($thumb, $path . "/" . $map . "-" . $mode . "-thumb.png", 9);
			imagedestroy($thumb);
		}
